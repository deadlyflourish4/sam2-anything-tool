...
import json
import os
from sam2.utils.mask2label import convert
...
class ImageManualPredictor:
    def __init__(self):
        self.model = None
        self.device = "cuda" if torch.cuda.is_available() else "cpu"

    def load_input_boxes(self, file_path):
        with open(file_path, 'r') as file:
            data = json.load(file)
        
        box_list = data["shapes"]
        bbox_list = []
        bbox_id = []
        for box in box_list:
            corner = np.array(box['points'])
            bbox = np.concat((corner[0], corner[2]))
            bbox_list.append(bbox)

            id = box["group_id"]
            bbox_id.append(id)

        return np.array(bbox_list), np.array(bbox_id)
    ...
    def image_predict(
        ...
        if input_box is None:
            input_box, boxid = self.load_input_boxes(label)
